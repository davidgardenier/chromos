...
def plot_allpcs():
    ...
    single_obj=[('J1701_462', 'XTE J1701-462')]
    ...
    for i, o in enumerate(single_obj):
        name = o[-1]
        o = o[0]
        p = path(o)
        db = pd.read_csv(p)
        ...
        x_single_obj.extend(hues)
        y_single_obj.extend(hardness)
        xerror_single_obj.extend(hues_err)
    ...
    g.plot(graph.data.values(x=x_single_obj, y=y_single_obj, dx=xerror_single_obj, dy=yerror_single_obj, title=name), errstyle)
    g.writePDFfile('/scratch/david/master_project/plots/publication/hh/ns_j1701')
