...
g2.plot(graph.data.function("y(x)=cos(20*x)*exp(-2*x)"))
        ...
        g1 = c.insert(graph.graphxy(height=6,
                                    width=6,
                                    ...
                                    y=graph.axis.log(min=0.01,
                                                     max=100,
                                                     title=r"PC2")))
        g1.plot(graph.data.values(x=x, y=y, dx=xerror, dy=yerror),
                [graph.style.symbol(symbolattrs=[colour],size=0.1),
                graph.style.errorbar(errorbarattrs=[colour])])
        g1.text(5.7,5.4, name, [text.halign.boxright])

        # TODO Adapt graphs to add in plots with http://pyx.sourceforge.net/examples/axis/link.html
        # Seee http://www.mpe.mpg.de/~simonbr/projects_pyxexamples/example_9.py
        g2 = c.insert(graph.graphxy(width=8, ypos=g1.height+0.5,
                                    x=graph.axis.linkedaxis(g1.axes["x"])))
        g2.plot(graph.data.function("y(x)=cos(20*x)*exp(-2*x)"))

        c.writePDFfile('/scratch/david/master_project/plots/publication/pc/individual/' + o)
        # Subplots
