...
rate, t, dt, n_bins, error = read_light_curve('/scratch/david/master_project/aquila_X1/P20092/20092-01-03-05/event_125us.lc')
rate = rate[0::10]
t = t - t[0]
t = t[0::10]
# Set up plot details
g = graph.graphxy(width=7,
                  x=graph.axis.lin(min=0,max=2000,title="Time (s)",density=1),
                  y=graph.axis.lin(min=0.01, max=6000, title="Rate (s$^{-1}$)"))

g.plot(graph.data.values(x=t,y=rate),[graph.style.line([style.linewidth(0.005), color.rgb.red])])

outputfile = '/scratch/david/master_project/plots/publication/presentation/lc'
