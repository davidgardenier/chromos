...
def power_colour(path):
    ...
    # Calculate what to exclude
    #----------------------------------
    variances_exclude = []
    variance_errors_exclude = []
    index_frequency_bands_exclude = []

    # Convert frequency bands to index values from in the frequency list
    for fb in [11.040,11.050]:
        index_exclude = min(range(len(frequency)), key=lambda i: abs(frequency[i]-fb))
        index_frequency_bands_exclude.append([index_exclude])

    # Group indexes into sets of style [low, high)
    for i, e in enumerate(index_frequency_bands_exclude[:-1]):
        e.append(index_frequency_bands_exclude[i+1][0]-1)

    del index_frequency_bands_exclude[-1]
    #--------------------------------------

    for i, e in enumerate(index_frequency_bands):
        variance = bin_width*sum(power_spectrum[e[0]:e[1]])
        if i == 3:
            variance -= bin_width*sum(power_spectrum[index_frequency_bands_exclude[0][0]:index_frequency_bands_exclude[0][1]])
        ...
        if i == 3:
            prop_std -= sum(power_spectrum_squared[index_frequency_bands_exclude[0][0]:index_frequency_bands_exclude[0][1]])
